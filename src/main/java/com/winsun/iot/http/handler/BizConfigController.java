...
public class BizConfigController implements HttpController {
    ...
    public void execute(HttpRequestWrapper request, HttpResponse resp) {
        ...
        CmdResult<String> result = dm.invokeCmd(topic, EnumQoS.AtleastOnce,msgType,baseId,cmdObj,new InnerCmdCallback(), 0, false, false);
        bizService.startBiz(result.getData(),baseId,obj.toString(),msgType, "config", EnumQoS.AtleastOnce.getCode());
        resp.write(result);
    }
