...
public class BizConfigController implements HttpController {
    ...
    public void execute(HttpRequestWrapper request, HttpResponse resp) {
        ...
        CmdResult<BizInfo> result = dm.invokeCmd(topic, EnumQoS.AtleastOnce,msgType,baseId,cmdObj,new InnerCmdCallback(), 0, false, false);
        bizService.startBiz(result.getData().getBizId(), 0, baseId,obj.toString(),msgType, "config", EnumQoS.AtleastOnce.getCode());
        resp.write(result);
    }
